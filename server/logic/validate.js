function validate(str) {
  ...
  if (newStr.length > 500) {
    return {
      outcome: false,
      text: "Please stay under 500 characters (excluding whitespace).",
    };
  }
  ...
  if (!okCharsRegex.test(newStr)) {
    return {
      outcome: false,
      text: 'Please remove any characters other than digits 0-9, ".", "+", "-", "*", "/", "(", ")", and space.',
    };
  }
  ...
  const seriesOperatorsRegex =
    /[\+\*\/\-][\+\*\/]|[\+\*\/\-]{3,}|\.[\+\*\/\-\.\(\)]/;
  if (seriesOperatorsRegex.test(newStr)) {
    return {
      outcome: false,
      text: "Please check for extraneous or missing operators or missing numbers.",
    };
  }
  ...
  if (firstLastRegex.test(newStr)) {
    return {
      outcome: false,
      text: "Please ensure the start and end are valid.",
    };
  }
  ...
  if (decimalPointRegex.test(newStr)) {
    return {
      outcome: false,
      text: "Please make sure each number has no more than one decimal point, and two decimal points are never adjacent.",
    };
  }
  ...
  const checkParens = (inputStr) => {
    ...
    for (let i = 0; i < inputStr.length; i++) {
      if (inputStr[i] === "(") {
        ...
        if (openParens < 0) {
          return {
            outcome: false,
            text: "Please make sure to have an open parenthesis before each close one.",
          };
        }
    ...
    if (openParens !== 0) {
      return {
        outcome: false,
        text: "Please be sure to close all open parentheses.",
      };
    }
    for (let i = 0; i < parensArray.length; i++) {
      ...
      if (current.end - current.start === 1) {
        return {
          outcome: false,
          text: "Please make sure that each set of parentheses enclose a statement.",
        };
      }
      ...
      if (current.start !== 0 && current.end !== inputStr.length - 1) {
        ...
        if (
          /\d\(/.test(
            inputStr.substring(current.start - 1, current.start + 1)
          ) &&
          /\)\d/.test(inputStr.substring(current.end, current.end + 2))
        ) {
          return {
            outcome: false,
            text: "Please make sure that each set of parentheses is next to only one number.",
          };
        }
      }
    }
