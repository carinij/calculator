...
function calc(inputString) {
  function evaluate(str) {
    ...
    function decideOperators(opIndex1, opIndex2) {
      if (opIndex1 !== -1) {
        if (opIndex2 !== -1) {
          newStr = applyOperator(
            newStr,
            opIndex1 < opIndex2 ? opIndex1 : opIndex2
          );
          return evaluate(newStr);
        } else {
  ...
  function resolveParens(str, initialIndex) {
    ...
    for (let i = initialIndex; i < str.length; i++) {
      ...
      if (newStr[i] === "(") {
        ...
        newStr = replaceGroup(
          newStr,
          openParensIndex + modifier,
          i + modifier,
          evaluate(newStr.slice(openParensIndex + 1 + modifier, i + modifier))
        );
        return newStr;
      }
    }
  ...
  function applyOperator(str, index) {
    ...
    return replaceGroup(
      newStr,
      firstOperand.termIndex,
      secondOperand.termIndex,
      result
    );
  }
